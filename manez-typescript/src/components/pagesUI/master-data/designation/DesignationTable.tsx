...
import {
  designationData,
  IDesignation,
} from "@/data/master-data/designationData";
...
const DesignationTable = () => {
  ...
  const [editData, setEditData] = useState<IDesignation | null>(null);
  ...
  const {
    ...
    handleSearchChange,
    setRows,
  } = useMaterialTableHook<IDesignation | any>(designationData, 10);
  ...
  useEffect(() => {
    const fetchDesignations = async () => {
      try {
        ...
        const mapped: IDesignation[] = array.map((item: any) => ({
          id: item.id,
          name: item.name,
  ...
  const handleDeleteDesignation = async (id: number) => {
    try {
      ...
      setRows((prev: IDesignation[]) => prev.filter((row) => row.id !== id));
    } catch {
      setError("Failed to delete designation");
    }
  };
    ...
    <>
      ...
      {modalDeleteOpen && (
        <DeleteModal
          ...
          handleDeleteFunc={handleDeleteDesignation}
          deleteId={deleteId}
        />
      )}
    </>
